...
const arrayWithoutElementAtIndex = function (arr: number[], index: number) {
...
{
    let ways = 0;
    let poweredNums: number[] = [];

    for (let i = 1; i < X + 1; i++) if (Math.pow(i, N) < X) poweredNums.push(Math.pow(i, N))
